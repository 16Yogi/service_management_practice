...
<body>
    ...
    <div class="container-fluid">
        <div class="container py-5">
            ...
            <?php
$select = "SELECT s.service_id, s.servicename, s.service_desc, s.service_img, IFNULL(ss.service_status, 0) AS service_status 
           FROM services s 
           LEFT JOIN service_status ss ON s.service_id = ss.service_id AND ss.customer_number = ?";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $status = (int)$row['service_status'];
?>
<div class="col-md-6 col-lg-12 mb-4 service-card" id="serviceCard<?= $row['service_id'] ?>">
    <div class="card">
        <div class="row no-gutters">
            <div class="col-md-8">
                <div class="card-body">
                    <h5 class="card-title"><?= htmlspecialchars($row['servicename']); ?></h5>
                    <p class="card-text"><?= htmlspecialchars($row['service_desc']); ?></p>
                    <p class="text-warning"><strong>Rs. 1500/-</strong></p>

                    <?php if ($status == 0): ?>
                        <button class="btn btn-info btn-sm book-btn" data-id="<?= $row['service_id'] ?>">Book Service</button>
                    <?php else: ?>
                        <button class="btn btn-danger btn-sm cancel-btn" data-id="<?= $row['service_id'] ?>">Cancel Booking</button>
            ...
            <div class="col-md-4">
                <img src="uploads/<?= htmlspecialchars($row['service_img']); ?>" class="card-img" alt="Service Image">
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function () {
    $(".book-btn").click(function () {
        ...
        $.ajax({
            url: "backend/book_service_ajax.php",
            type: "POST",
            data: { action: "book", service_id: serviceId },
            success: function (res) {
                $("#serviceCard" + serviceId).html(res);
            },
            error: function () {
    ...
    $(".cancel-btn").click(function () {
        ...
        $.ajax({
            url: "backend/book_service_ajax.php",
            type: "POST",
            data: { action: "cancel", service_id: serviceId },
            success: function (res) {
                $("#serviceCard" + serviceId).html(res);
            },
            error: function () {
